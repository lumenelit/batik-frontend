...
export default function PageIndustri() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [industriRes, imageRes] = await Promise.all([
                    api.get(`/industri/${idIndustri}`),
                    api.get(`/industri/image/${idIndustri}`)
                ]);
                setIndustri(industriRes.data.data[0]);
                setDescription(
                    lang === "en"
                        ? await translateToEnglish(
                              industriRes.data.data[0].desc
                          )
                        : industriRes.data.data[0].desc
                );
                setIndustriImage(imageRes.data.data[0]);
            } catch (error) {
